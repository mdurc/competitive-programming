// use as KdTree* root = nullptr, passing this as root as arg1
// useful for counting n-dimension points in a region
struct KdTree {
  ...
  KdTree(int x, int y, int val)
      : x(x), y(y), val(val), sum(val), sz(1), x_max(x), x_min(x),
        y_max(y), y_min(y), ch{} {}
  void push_up() {
    sz = 1;
    sum = val;
...
// returns the sum of the coordinates within the rectangle provided
...
// insert where A is the value, rooted at P
