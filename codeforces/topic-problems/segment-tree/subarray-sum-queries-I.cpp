...
// dp solution is too slow O(n) after each query
ll mxsub(vector<int>& a) {
  int n = a.size(), mx = 0, curr = 0;
  for (int i = 0; i < n; ++i) {
...
// segment tree solution is faster O(log n) after each query
struct SGT {
  struct Node {
...
int main() {
  ...
  for (int i = 0; i < m; ++i) {
    ...
    cin >> k >> x;
    sg.change(k - 1, x);
    cout << sg.query(1, 0, sg.n - 1, 0, n-1).g_max << "\n";
  }
