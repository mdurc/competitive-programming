...
* [Binary search](binary-search.md) requires a **monotonic** search space. If a function increases to a peak and then decreases (or decreases to a valley and then increases), binary search cannot be directly applied to find that peak/valley.
  * This type of curve is called a **unimodal function**. It has exactly one extreme point (maximum or minimum).

- **Plan:**
  - If we pick two points $m_1$ and $m_2$ inside our search space $[l, r]$ such that $l < m_1 < m_2 < r$, we can evaluate $f(m_1)$ and $f(m_2)$.
