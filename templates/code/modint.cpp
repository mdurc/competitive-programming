...
struct Mod {
  ...
  using UV = std::conditional_t<sizeof(U) <= 4, ull, u128>;
  using SV = std::conditional_t<sizeof(U) <= 4, ll, i128>;
  constexpr Mod() : x(0) {}
  ...
  constexpr bool operator<=(const Mod& rhs) const { return x <= rhs.x; }
  constexpr bool operator>=(const Mod& rhs) const { return x >= rhs.x; }
  friend constexpr std::istream& operator>>(std::istream& is, Mod& a) { ll i = 0; is >> i; a = i; return is; }
...
struct DynMod {
  ...
  constexpr bool operator<=(const DynMod& rhs) const { return x <= rhs.x; }
  constexpr bool operator>=(const DynMod& rhs) const { return x >= rhs.x; }
  friend constexpr std::istream& operator>>(std::istream& is, DynMod& a) { ll i = 0; is >> i; a = i; return is; }
...
template<typename U, uint Id>
Barrett<U> DynMod<U, Id>::bt = MOD_INT;

using Mint = Mod<uint, MOD_INT>;
using DMint = DynMod<uint, 0>;
